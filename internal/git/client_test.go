...
func TestClient_FlagInjectionPrevention(t *testing.T) {
	mock := NewMockRunner()
	ctx := context.Background()
	repoDir := "C:/test/repo"
	c := NewClient(mock)

	// GetDiffBetween with flag injection
	_, err := c.GetDiffBetween(ctx, repoDir, "--output=pwn.txt", "commit2")
	if !errors.Is(err, ErrInvalidCommit) {
		t.Errorf("expected ErrInvalidCommit on flag fromCommit, got: %v", err)
	}

	// GetChangedFilesBetween with flag injection
	_, err = c.GetChangedFilesBetween(ctx, repoDir, "commit1", "-U0")
	if !errors.Is(err, ErrInvalidCommit) {
		t.Errorf("expected ErrInvalidCommit on flag toCommit, got: %v", err)
	}

	// CommitExists with flag injection
	exists, err := c.CommitExists(ctx, repoDir, "--exec=whoami")
	if err != nil || exists {
		t.Errorf("expected CommitExists=false, nil for flag commit, got %v, %v", exists, err)
	}

	// IsAncestor with flag injection
	isAnc, err := c.IsAncestor(ctx, repoDir, "--leak", "commit2")
	if !errors.Is(err, ErrInvalidCommit) || isAnc {
		t.Errorf("expected ErrInvalidCommit on flag ancestor, got %v, %v", isAnc, err)
	}
}
